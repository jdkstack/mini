...
public class TaskWorker<T> implements Runnable {
  ...
  @Override
  public void run() {
    this.dispatch(this.event, this.handler);
  }
  ...
  public Worker<T> getHandler() {
    return this.handler;
  }

  public void setHandler(final Worker<T> handler) {
   ...
   * @author admin
   */
  public final void dispatch(final T event, final Worker<T> handler) {
    try {
      // 增加开始时间.
      this.beginDispatch();
      // 执行具体业务,传递event参数.
