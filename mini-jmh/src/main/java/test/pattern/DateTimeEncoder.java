 ...
 * @author admin
 */
public final class DateTimeEncoder extends LogEventPatternConverter implements ArrayPatternConverter, Encoder {
  ...
  private DateTimeEncoder(final String[] options) {
    super("Date", "date");
    System.out.println(options.length);
  }

  public static DateTimeEncoder newInstance(final String[] options) {
    return new DateTimeEncoder(options);
  }
  ...
  public void format(LogRecord event, StringBuilder toAppendTo) {
    encoder(toAppendTo, System.currentTimeMillis(), TimeZone.EAST8);
  }
}
